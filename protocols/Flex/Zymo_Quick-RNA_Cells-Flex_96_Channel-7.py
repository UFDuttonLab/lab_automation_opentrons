...
metadata = {
    'protocolName': 'Zymo Quick-RNA Cells — Flex 96-Channel',
    'author': 'Dutton Lab (adapted from Zach Galluzzo, Opentrons)',
    'description': 'Magnetic bead RNA extraction using Zymo Quick-RNA MagBead kit. '
                   '96-channel head, heater-shaker mixing, on-bead DNase I treatment.'
}
...
def add_parameters(parameters: protocol_api.Parameters):
    """Runtime parameters — shown in the Opentrons App UI before the run."""
    parameters.add_bool(
        variable_name="DRYRUN",
        display_name="Dry Run",
        description="Skip incubations, shorten shaking, return tips. For testing only.",
        default=True
    )
    parameters.add_bool(
        variable_name="HEATER_SHAKER",
        display_name="Use Heater-Shaker",
        description="Enable heater-shaker for mixing. Disable for manual shaking.",
        default=True
    )
    parameters.add_bool(
        variable_name="TIP_MIXING",
        display_name="Tip Mixing (no H-S fallback)",
        description="Pipette tip mixing when H-S is off. Ignored when H-S is enabled.",
        default=False
    )
...
def run(ctx):
    """
    Zymo Quick-RNA Cells extraction protocol.
    Deck layout (Robin with 96-channel head):
    #        1             2              3              4
    #   A: [tip_rack_1]   [tip_rack_2]   temp_mod       —
    #   B: wash3-6_res    stop_res       dnase_plate    —
    #   C: mag_block      wash1_res      bind_res       —
    #   D: heater_shaker  lysis_res→D2   [waste_chute]  [flex_stacker]
    """
    # ── Read runtime parameters ──
    dry_run         = ctx.params.DRYRUN
    heater_shaker   = ctx.params.HEATER_SHAKER
    tip_mixing      = ctx.params.TIP_MIXING
    USE_GRIPPER     = True
    trash_chute     = True
    temp_mod        = True

    # Kit-specific volumes (Zymo Quick-RNA MagBead)
    sample_vol      = 300
    lysis_vol       = 200
    bind_vol        = 400
    wash_vol        = 400
    ...
    # Heater-shaker overrides tip mixing
    if heater_shaker:
        tip_mixing  = False
    ...
    # Flex Stacker installed in D4 (adjacent to waste chute in D3)
    flex_stacker        = ctx.load_module('flexStackerModuleV1','D4')
    ...
    def lysis(vol, source):
        ...
        if heater_shaker:
            ...
            bead_mix(vol,samples_m,reps=5 if not dry_run else 1)
            pip.return_tip()
        else:
            pip.return_tip()
            if not dry_run:
        ...
        #Discard spent lysis reservoir through waste chute
        ctx.move_labware(
            lysis_reservoir,
            trash if USE_GRIPPER else protocol_api.OFF_DECK,
            use_gripper=USE_GRIPPER
        )
    ...
    def bind(vol,source):
        ...
        if heater_shaker:
            ...
            for x in range(3 if not dry_run else 1):
                bead_mix(binding_buffer_vol,samples_m,reps=10 if not dry_run else 1)
    ...
    def wash(vol, source):
        ...
        if heater_shaker:
            ...
            bead_mix(vol,samples_m,reps=12 if not dry_run else 1)
            pip.return_tip()
        else:
            pip.return_tip()
            if not dry_run:
    ...
    def dnase(vol, source):
        ...
        if heater_shaker:
            ...
            for x in range(2 if not dry_run else 1):
                bead_mix(vol,samples_m,reps=10 if not dry_run else 1)
                ctx.delay(minutes=1 if not dry_run else 0.25)
            pip.return_tip()
        else:
            pip.return_tip()
            if not dry_run:
    ...
    def elute(vol,source):
        ...
        if heater_shaker:
            ...
            bead_mix(vol,samples_m,reps=9 if not dry_run else 1)
            pip.return_tip()
            if not dry_run and temp_mod:
                tempdeck.set_temperature(4)
        else:
